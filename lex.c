...
static char *
readfile(const char *filename, long *filesize) {
...
int
lexer_from_buf(struct lexer *l, const char *buf, size_t len) {
	memset(l, 0, sizeof(struct lexer));
...
static void
maketoken(struct lexer *l, enum token_type type) {
...
static void
lex_ident_or_keyword(struct lexer *l) {
	// multi-char keywords
...
// position `pos`.
struct src_loc
locate(struct source *src, size_t pos) {
	// TODO: performance
...
char *
tokenstr(const char *src, struct token tok, char *buf, size_t buflen) {
...
int
tokeneq(const char *src, struct token t1, struct token t2) {
...
char *
tokentypestr(enum token_type t, char *buf, size_t buflen) {
	if (t >= TASCII)
...
void
tokenprint(const char *src, const struct token tok) {
	char buf[TOKLEN];
